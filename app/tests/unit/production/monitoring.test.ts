...
import {
  ...
  IMonitoring,
} from "../../../src/production/monitoring";
import {
...
describe("ProductionMonitoring", () => {
  ...
  describe("Health Status", () => {
    ...
    it("should check component health accurately", () => {
      ...
      const toolComponent = healthStatus.components.find(
        (c) => c.name === "tool_operations"
      );
    ...
    it("should detect memory issues", () => {
      ...
      const memoryComponent = healthStatus.components.find(
        (c) => c.name === "memory_usage"
      );
    ...
    it("should monitor response times", () => {
      ...
      const responseComponent = healthStatus.components.find(
        (c) => c.name === "response_times"
      );
  ...
  describe("Metrics Summary", () => {
    ...
    it("should filter metrics by time window", () => {
      // Test that getMetricsSummary filters metrics older than 1 hour
      const oldTimestamp = Date.now() - 2 * 60 * 60 * 1000; // 2 hours ago
      
      // Manually inject old metric by accessing private field
      const toolMetric = {
        toolName: "Read",
      ...
      (monitoring as any).toolMetrics.push(toolMetric);
      
      // Also add a recent metric
      monitoring.recordToolOperation("Write", 150, true);

      const summary = monitoring.getMetricsSummary();
      
      // Should only count the recent metric, not the old one
      expect(summary.tools.totalCalls).toBe(1);
      expect(summary.tools.callsByTool.Write).toBe(1);
      expect(summary.tools.callsByTool.Read).toBeUndefined();
    });
  });
  ...
  describe("Integration with Constants", () => {
    ...
    it("should apply monitoring thresholds correctly", () => {
      ...
      expect(toolComponent!.details.errorRate).toBeGreaterThan(
        PRODUCTION_MONITORING.ERROR_RATE_THRESHOLD
      );
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle cleanup operations correctly", () => {
      // Create monitoring with very short retention for testing
      const shortRetentionMonitoring = new ProductionMonitoring(mockLogger, {
        metricsRetentionMs: 1, // 1ms retention
      });

      // Record metrics
      shortRetentionMonitoring.recordToolOperation("Read", 100, true);
      
      // Wait a bit to ensure metric expires
      const start = Date.now();
      ...
      (shortRetentionMonitoring as any).cleanupOldMetrics();
