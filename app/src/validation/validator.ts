...
export class ParameterValidator {
  ...
  static validateRequest(request: ChatCompletionRequest): ValidationResult {
    ...
    if (request.tools) {
      const toolErrors = this.validateTools(request.tools);
      errors.push(...toolErrors);
    }

    // Validate tool_choice if present
    if (request.tool_choice && request.tools) {
      const toolChoiceErrors = this.validateToolChoice(request.tool_choice, request.tools);
   ...
   */
  static validateTools(tools: any[]): string[] {
    const errors: string[] = [];
   ...
   */
  static validateToolChoice(toolChoice: any, tools: any[]): string[] {
    const errors: string[] = [];
